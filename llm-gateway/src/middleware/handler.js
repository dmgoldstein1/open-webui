...
const { getConversation, appendMessages } = require('../shared/historyStore');
...
async function postChatSync(body) {
  ...
  let assistantText = '';
  if (route === 'ollama') {
    assistantText = await callOllama(history.messages, user_message, images);
  } else {
    assistantText = await callVLLM(history.messages, user_message);
  }
  ...
  return makeResponse(200, {
    ...
    model: route === 'ollama' ? 'ollama' : 'vllm',
    choices: [{
      index: 0,
...
async function handler(event) {
  ...
  try {
    ...
    if (method === 'POST' && path.endsWith('/chat/sync')) {
      const body = typeof event.body === 'string' ? JSON.parse(event.body || '{}') : (event.body || {});
      return await postChatSync(body);
    }
    if (method === 'POST' && path.endsWith('/chat-sync')) {
      const body = typeof event.body === 'string' ? JSON.parse(event.body || '{}') : (event.body || {});
      return await postChatSync(body);
